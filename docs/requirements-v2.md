...
- NLU：闭集指令理解。默认规则快路径（整句只命中一个规则意图、去掉语气词/连接词后无残余、且无否定/纠正）跳过 LLM；「停/停止/急停」无否定时永远立刻停车。开关可关规则，关则非停止句全走云端大模型（默认官方 DeepSeek-flash，界面可选 chat / v4-pro 与 SiliconFlow V3.2 / V3 / Qwen3.5-27B / GLM-5.1）。LLM 只输出最终要执行的白名单指令（含否定/纠正/多动作，最多 1～8 条可配）；数值事后校正（高度毫米，n≤10 视为米；叉高/速度夹紧）。`{"intents":[]}` 静默不执行；超时/非 JSON 整句不执行并播「没听清，请再说一次」，禁止用规则首命中顶上。任务流 `FLOW_START` 仍二次确认。
...
ASR/NLU/TTS 联网增强（PTT 音频与意图抽取走云端，规则 NLU 与 piper 仍可离线）；出网内容为语音与模板话术文本；日志记录文本不录音频，7 天删除；配对/现场码见 2.5。所选 ASR/LLM 模型与 NLU 快路径开关写入 `config/runtime_models.yaml`。
